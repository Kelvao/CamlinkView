/*
 * menu_font.h — Fonte bitmap 5×7 para o menu OSD
 *
 * Cobre ASCII 32–126. Indexação: MFONT[c - 32].
 * Cada entrada = 5 bytes (colunas), bit0 = linha superior.
 *
 * Inclua em APENAS UMA unidade de compilação com MENU_FONT_IMPLEMENTATION
 * definido (menu.c); demais arquivos recebem declarações extern.
 */
#pragma once
...
#define MFONT_W   5
#define MFONT_H   7
#define MFONT_GAP 1                              /* gap entre colunas (px lógicos) */

#ifdef MENU_FONT_IMPLEMENTATION

/*
 * Fonte 5×7 — 5 colunas, 7 linhas, bit0 = linha superior (máscara 0x7F).
 * Cada coluna é um byte; bits 0–6 = linhas top→bottom.
 *
 *  Bit layout de uma coluna:
 *    bit6 bit5 bit4 bit3 bit2 bit1 bit0
 *    row6 row5 row4 row3 row2 row1 row0   (row0 = topo)
 */
const uint8_t MFONT[][MFONT_W] = {
...
/* Retorna índice seguro: ASCII imprimível → c-32; else → 0 (' '). */
int
mfont_glyph_idx (char c)
{
    unsigned char uc = (unsigned char)c;
    if (uc >= 32 && uc <= 126) return uc - 32;
    return 0;
}

#else

extern const uint8_t MFONT[][MFONT_W];
int mfont_glyph_idx (char c);

#endif /* MENU_FONT_IMPLEMENTATION */
